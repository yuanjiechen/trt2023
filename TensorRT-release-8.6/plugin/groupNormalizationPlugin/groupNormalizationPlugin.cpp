...
constexpr char const* kGROUP_NORM_NAME{"GroupNormalizationPlugin2"};
} // namespace
    ...
    cudnnContext* cudnnContext, cublasContext* cublasContext, IGpuAllocator* gpuAllocator) noexcept
{
    try
    {
        ...
        PLUGIN_CUDNNASSERT(cudnnCreateTensorDescriptor(&mTensorDesc2));
    ...
    int32_t pos, nvinfer1::PluginTensorDesc const* inOut, int32_t nbInputs, int32_t nbOutputs) noexcept
{
    try
    {
        ...
        // std::cout << pos  << "  " << int32_t(inOut[pos].type) << "  " << int32_t(inOut[pos].format) << "\n";
        return (((inOut[pos].type == nvinfer1::DataType::kFLOAT) || (inOut[pos].type == nvinfer1::DataType::kHALF)) && (inOut[pos].type == inOut[0].type) && (inOut[pos].format == TensorFormat::kLINEAR)); //|| inOut[pos].format == TensorFormat::kCHW2 || inOut[pos].format == TensorFormat::kHWC8 || inOut[pos].format == TensorFormat::kCHW4 || inOut[pos].format == TensorFormat::kCHW32 || inOut[pos].format == TensorFormat::kHWC16)
    ...
    nvinfer1::DynamicPluginTensorDesc const* out, int32_t nbOutputs) noexcept
{
    try
    {
        ...
        // std::cout << int32_t(inputType) << endl;
        ...
        // std::cout << int32_t(inputType) << "  end config" << endl;
