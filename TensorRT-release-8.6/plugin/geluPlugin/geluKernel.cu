...
__global__ void geluKernel(const T a, const T b, const T c, int n, const T* input, const T* x, T* output)
{
    ...
    if (idx < n)
    {
        ...
        output[idx] = xin * in * cdf;
    }
}

int computeGelu(cudaStream_t stream, int n, const float* input, const float* x, float* output)
{
    constexpr int blockSize = 256;
    const int gridSize = (n + blockSize - 1) / blockSize;
    geluKernel<float, blockSize><<<gridSize, blockSize, 0, stream>>>(A, B, C, n, input, x, output);

    PLUGIN_CHECK(cudaPeekAtLastError());
    return 0;
}

int computeGelu(cudaStream_t stream, int n, const half* input, const half* x, half* output)
{
    constexpr int blockSize = 256;
    ...
    if (0 == (n & 1))
    {
        ...
        geluKernel<half2, blockSize><<<gridSize, blockSize, 0, stream>>>(A2, B2, C2, n2, input2, x2, output2);
    }
    else
    {
        const int gridSize = (n + blockSize - 1) / blockSize;
        geluKernel<half, blockSize><<<gridSize, blockSize, 0, stream>>>(A, B, C, n, input, x, output);
    }
